...
| Instance protection | `disable_api_termination` prevents accidental deletion |
...
- **Instance termination protection:** `disable_api_termination = true` prevents accidental deletion via the console or API. To destroy the infrastructure with `terraform destroy`, you must first set `disable_api_termination = false` in `main.tf` and run `terraform apply`.
